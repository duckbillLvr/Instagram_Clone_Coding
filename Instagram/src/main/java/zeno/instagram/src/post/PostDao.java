...
public class PostDao {
    ...
    public List<GetPostsRes> selectPosts(int userIdx) {
        String selectUserPostsQuery = "SELECT p.postIdx as postIdx,\n" +
                ...
                "        case\n" +
                "            when timestampdiff(second, p.updateAt, current_timestamp) < 60\n" +
                "            then concat(timestampdiff(second, p.updateAt, current_timestamp), '초 전')\n" +
                "            when timestampdiff(minute , p.updateAt, current_timestamp) < 60\n" +
                "            then concat(timestampdiff(minute, p.updateAt, current_timestamp), '분 전')\n" +
                "            when timestampdiff(hour , p.updateAt, current_timestamp) < 24\n" +
                "            then concat(timestampdiff(hour, p.updateAt, current_timestamp), '시간 전')\n" +
                "            when timestampdiff(day , p.updateAt, current_timestamp) < 365\n" +
                "            then concat(timestampdiff(day, p.updateAt, current_timestamp), '일 전')\n" +
                "            else timestampdiff(year , p.updateAt, current_timestamp)\n" +
                "        end as updatedAt,\n" +
                ...
                "WHERE f.followerIdx = ? and p.status = 'ACTIVE' group by p.postIdx";
